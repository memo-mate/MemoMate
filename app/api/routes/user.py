...
from app.enums import AccessRole
...
def get_user_info(current_user: CurrentUser) -> Any:
    if not current_user.is_active:
        role = AccessRole.INACTIVE
    elif current_user.is_superuser:
        role = AccessRole.SUPER
    else:
        role = AccessRole.ACTIVE

    return UserInfo(roles=[role], realName=current_user.username)
...
def update_user(
    *,
    ...
    if user_in.email:
        existing_user = crud.get_user_by_username_or_email(session=session, username=user_in.email)
